...
def span_rank_int(vectors):
 ...
 return len(piv)
# type3 order32
t0=time.time();ti=3;pr=min(ORBS[ti]);G=[M for M in CINV if qact(pr,M)==pr];q=len(G);assert q==32
...
out={'phase':'II-KM','type_index':ti,'group_order':q,'target_native_H5_dimension':target,'elementary_abelian_E8_subgroups_found':len(subs),'subgroups_used':len(details),'candidate_family':{'H1_H2_H2':nA,'H1_H1_H3':nB,'H1_H3_H1':nC,'H2_H3':nD,'H3_H2':nE,'total':nc},'joint_augmented_restriction_certificate':{'candidate_pivots':cand_piv,'rows_processed':rows_total,'subgroups':details},'closure_logic':'the augmented matrix has shared candidate columns and an independent B5 block for each E8 subgroup; candidate-pivot rank is exactly the rank of the direct-sum restriction images modulo direct-sum coboundaries, hence a lower bound for the native product span in H5(G). Reaching the independent abstract H5 dimension closes the product span.','decision':'PASS_TYPE3_ORDER32_NATIVE_H5_PRODUCT_CLOSURE_BY_MULTI_E8_RESTRICTION' if closed else 'SEPARATED_TYPE3_NOT_CLOSED_BY_E8_RESTRICTION','guards':['all subgroup restrictions use actual native H1/H2/H3 bar cocycles','each B5 block is the actual inhomogeneous coboundary C4(E)->C5(E)','no restriction rank is transferred to another stabilizer type','closure uses only image-rank lower bound plus the independent H5 dimension upper bound'],'runtime_seconds':round(time.time()-t0,3),'status':'PASS_THIRD_NATIVE_H5_B5_CLOSURE_TYPE3' if closed else 'PASS_E8_RESTRICTION_PROGRESS_TYPE3_REMAINS_SEPARATED'}
(Path('/mnt/data/II_KM/II_KM_golden_native_H5_type3.json')).write_text(json.dumps(out,indent=2,sort_keys=True)+'\n')
